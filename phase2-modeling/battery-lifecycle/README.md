...
> 📷 **建议插图**：数据结构示意图，展示 JSON 嵌套层级与 discharge/charge 分块存储方式
...
为确认对数据结构的理解正确，从数据库中提取了单循环的充放电数据并绘制成曲线。下图展示了第 50 循环的放电曲线（Q-V）及其微分容量曲线（dQ/dV），结果与典型 LFP 电池特性吻合，验证了数据提取流程的正确性。

![数据验证：充放电曲线与dQ/dV](../../dqdv_charge_discharge.png)
...
> 📷 **建议插图**：ΔQ(V) 曲线示例，展示健康电池与早衰电池的差分曲线对比
...
> 📷 **建议插图**：交叉验证热力图，展示不同 α 和 l1_ratio 组合下的验证误差
...
> 📷 **建议插图**：预测值 vs 真实值散点图，展示模型拟合效果与误差分布
